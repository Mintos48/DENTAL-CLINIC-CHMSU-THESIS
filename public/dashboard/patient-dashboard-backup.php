...
<body>
    <div class="dashboard-background">
        ...
        <div class="dashboard-container">
            ...
            <!-- Profile Section -->
            <div class="section-card" id="profile-section" style="display: none;">
                <div class="section-header">
                    <h3>
                            ...
                            <div class="profile-details">
                                ...
                                <div class="detail-item">
                                    ...
                                    <div>
                                        ...
                                        <span id="profile-last-login">Just now</span>
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    <script>
        ...
        class PatientDashboard {
            ...
            testReferralController() {
                console.log('=== DEBUG: Testing ReferralController endpoint ===');
                ...
                .then(text => {
                    console.log('=== DEBUG: Test response text ===', text);
                    try {
                        const data = JSON.parse(text);
                        console.log('=== DEBUG: Test endpoint success ===', data);
                        return data;
                    } catch (e) {
                        console.error('=== DEBUG: Test endpoint returned non-JSON ===', text);
                        throw e;
                    }
                })
                .catch(error => {
                    console.error('=== DEBUG: Test endpoint failed ===', error);
                    throw error;
                });
            }
            ...
            loadClinicHours(specificClinicId = null) {
                ...
                if (!clinicId) {
                    if (this.selectedClinicId) {
                        ...
                        console.log('🏥 Using selected clinic ID for hours:', clinicId);
                    } else {
                        // Fallback to user's branch (legacy support)
                        clinicId = this.getCurrentUserBranchId();
                        console.log('🏠 Using user branch ID for hours:', clinicId);
                    }
                }
                
                if (!clinicId) {
                    console.log('🌐 No clinic ID available, showing general hours view');
                    this.setGeneralClinicHoursView();
                    return;
                }
                
                console.log('⏰ Loading clinic hours for clinic ID:', clinicId);
            ...
            loadReferralStatus() {
                console.log('=== DEBUG: loadReferralStatus called ===');
                ...
                .then(text => {
                    ...
                    try {
                        ...
                        if (data.success && data.hasReferral) {
                            console.log('=== DEBUG: Has referral, calling displayReferralStatus ===');
                            this.displayReferralStatus(data.referral);
                        } else {
                            console.log('=== DEBUG: No referral found, hiding section ===');
                            // Hide referral status section if no active referrals
                            document.getElementById('referral-status').style.display = 'none';
                        }
                    } catch (parseError) {
                        console.error('=== DEBUG: JSON parse error ===', parseError);
                        console.log('=== DEBUG: Response was not valid JSON ===', text);
                        document.getElementById('referral-status').style.display = 'none';
                    }
                })
                .catch(error => {
            ...
            extractTreatmentName(referral) {
                // Debug logging
                console.log('Extracting treatment name from referral:', {
                    treatment_name: referral.treatment_name,
                    notes: referral.notes,
                    treatment_type_id: referral.treatment_type_id
                });
                
                // First check if treatment_name exists directly
                if (referral.treatment_name) {
                    console.log('Found direct treatment_name:', referral.treatment_name);
                ...
                if (referral.notes && referral.notes.includes('Treatment requested:')) {
                    ...
                    if (match && match[1]) {
                        ...
                        console.log('Extracted treatment from notes (Treatment requested):', extractedName);
                ...
                if (referral.notes) {
                    ...
                    for (const pattern of treatmentPatterns) {
                        ...
                        if (match && match[1]) {
                            const extractedName = match[1].trim();
                            console.log('Extracted treatment from notes (pattern match):', extractedName);
                            return extractedName;
                        }
                    }
                }
                ...
                console.log('No treatment name found, using fallback: General consultation');
            ...
            displayReferralStatus(referral) {
                // Debug logging
                console.log('=== DEBUG: displayReferralStatus called ===');
                console.log('Referral object:', referral);
                console.log('Referral status:', referral.status);
                console.log('Status type:', typeof referral.status);
                console.log('Status length:', referral.status ? referral.status.length : 'undefined');
                console.log('Status === "pending_patient_approval":', referral.status === 'pending_patient_approval');
                ...
                console.log('=== DEBUG: Selected config ===', config);
                ...
                console.log('=== DEBUG: Checking referral status ===');
                console.log('normalizedStatus:', normalizedStatus);
                console.log('Trimmed status:', normalizedStatus);
                
                if (normalizedStatus === 'pending_patient_approval') {
                    console.log('=== DEBUG: Status matches pending_patient_approval - rendering buttons ===');
                    detailsEl.innerHTML = `
                    ...
                    console.log('=== DEBUG: Setting actionsEl display to block ===');
                    ...
                    console.log('=== DEBUG: Button HTML set ===', buttonHtml);
                    console.log('=== DEBUG: actionsEl after update ===', actionsEl);
                    
                } else if (normalizedStatus === 'patient_approved') {
                    console.log('=== DEBUG: Status is patient_approved - hiding actions ===');
            ...
            loadUserInfo() {
                ...
                .then(data => {
                    if (data.success && data.user) {
                        ...
                        console.log('👤 Loaded user info (branch-independent):', { 
                            name: user.name, 
                            email: user.email, 
                            role: user.role 
                        });
                        ...
                        if (branchNameEl) {
                            ...
                            if (selectedClinic && this.selectedClinicId) {
                                ...
                                console.log('🏥 Showing selected clinic in branch info:', selectedClinic.name);
                            } else {
                                ...
                                console.log('🌐 Showing general network membership');
                            }
                        }
                        
                        // Only update navbar if no clinic is currently selected
                        if (!this.selectedClinicId) {
                            console.log('🌐 No clinic selected, setting general navbar branding');
                            this.setGeneralNavbarBranding();
                        } else {
                            console.log('🏥 Clinic already selected, preserving clinic-specific navbar');
                        }
            ...
            loadProfileInfo() {
                ...
                document.getElementById('profile-branch').textContent = user.branch_name;
                
                // Set member since date from API data
            ...
            updateProfileStats() {
                ...
                document.getElementById('profile-total-appointments').textContent = total;
                document.getElementById('profile-completed-appointments').textContent = completed;
                document.getElementById('profile-pending-appointments').textContent = pending;
            }
            ...
            loadAppointments() {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        this.updateProfileStats(); // Update profile statistics
                    } else {
            ...
            loadTreatmentTypes() {
                console.log('loadTreatmentTypes called (initial load, no specific clinic)');
                fetch('../../src/controllers/TreatmentController.php?action=getTreatmentTypes')
                .then(response => response.json())
                .then(data => {
                    console.log('TreatmentController response (initial):', data);
                    if (data.success) {
            ...
            loadTreatmentsForClinic(clinicId) {
                console.log('loadTreatmentsForClinic called with clinicId:', clinicId);
                ...
                console.log('Fetching treatments from:', apiUrl);
                
                fetch(apiUrl)
                .then(response => response.json())
                .then(data => {
                    console.log('TreatmentController response for clinic', clinicId, ':', data);
            ...
            loadDynamicTimeSlots(date) {
                ...
                if (this.selectedClinicId && this.selectedClinicId !== 'null' && this.selectedClinicId !== '') {
                    ...
                    console.log('Using selected clinic for time slots:', branchId);
                } 
                // Second priority: hidden clinic field value (if set)
                    ...
                    if (hiddenClinicField && hiddenClinicField.value && hiddenClinicField.value !== '') {
                        ...
                        console.log('Using hidden clinic field value for time slots:', branchId);
                    }
                    // Last resort: user's home clinic
                        ...
                        console.log('Falling back to user home clinic for time slots:', branchId);
                    } else {
            ...
            populateTimeSlots(slots, dayInfo, treatmentDuration = 60) {
                ...
                // Add available slots with duration information
                slots.forEach(slot => {
                    const option = document.createElement('option');
                    ...
                    if (slot.available) {
                        ...
                        option.textContent = `${slot.display} - ${formattedEndTime} (${this.formatDuration(treatmentDuration)})`;
                        option.disabled = false;
                        option.style.color = '';
                    } else {
                        // Show why slot is not available
                        let reason = 'Booked';
                        ...
                        option.textContent = `${slot.display} ❌ (${reason})`;
            ...
            async fetchAppointmentPrice(appointment) {
                ...
                if (appointment.treatment_type_id && appointment.branch_id) {
                    try {
                        ...
                        console.log('Error fetching treatment price:', error);
                    }
                }
                    ...
                    <tr${isReferredAppointment ? ' style="background: linear-gradient(90deg, #fff3cd 0%, #ffffff 100%);"' : ''}>
                        ...
                        <td>
                            <div class="action-group">
                                <button class="btn btn-info btn-small" onclick="dashboard.viewAppointmentHistory(${appointment.id})" style="margin-right: 8px;">
            ...
            // Helper function to view appointment history
            ...
            startPeriodicRefresh() {
                ...
                setInterval(() => {
                    ...
                    if (timeSinceLastNotification > 25000) { // If no notification refresh in last 25 seconds
                        console.log('📅 Periodic refresh - loading appointments and referral status...');
                        this.loadAppointments();
                        this.loadReferralStatus();
                    } else {
                        console.log('📅 Skipping periodic refresh - recent notification refresh detected');
                    }
                }, 30000);
            ...
            onNotificationTriggeredRefresh() {
                this.lastNotificationRefresh = Date.now();
                console.log('🔔 Dashboard refreshed due to new notifications');
            ...
            handleClinicSelection() {
                ...
                console.log('🏥 handleClinicSelection called with:', { clinic, branchId, action });
                ...
                if ((clinic || branchId) && action === 'book') {
                    ...
                    if (branchId) {
                        clinicId = branchId;
                        console.log('🎯 Using branch_id from URL:', clinicId);
                        ...
                        if (clinicMap[clinic]) {
                            ...
                            console.log('Mapped clinic name to info:', clinic, '->', clinicMap[clinic]);
                        }
                    }
                    ...
                    if (clinicId && clinicName) {
                        ...
                        console.log('💾 Stored clinic in session storage:', selectedClinicData);
                        
                        // Store the selected clinic ID for time slot loading
                        this.selectedClinicId = clinicId;
                        console.log('🎯 Set selectedClinicId to:', this.selectedClinicId);
            ...
            updateNavbarBranding(clinicName) {
                console.log('🎨 updateNavbarBranding called with clinicName:', clinicName);
                ...
                console.log('🔍 Found elements:', { 
                    ...
                    navbarClinicNameEl: !!navbarClinicNameEl,
                    currentLogoSrc: navbarLogoImg ? navbarLogoImg.src : 'N/A'
                });
                ...
                console.log('🏥 Clinic branding mapping:', { clinicName, branding: !!branding, expectedLogo: branding?.logo });
                
                if (branding) {
                    console.log('✅ Branding found, starting logo update...');
                    ...
                    console.log('🔄 Opacity set to 0.5, scheduling logo change...');
                    
                    setTimeout(() => {
                        console.log('⏰ Timeout executed, updating logo and name...');
                        console.log('📝 Before update:', { src: navbarLogoImg.src, name: navbarClinicNameEl.innerHTML });
                        
                        navbarLogoImg.src = branding.logo;
                        navbarLogoImg.alt = branding.alt;
                        navbarClinicNameEl.innerHTML = branding.displayName;
                        
                        console.log('📝 After update:', { src: navbarLogoImg.src, name: navbarClinicNameEl.innerHTML });
                        ...
                        console.log('✨ Updated navbar branding for:', clinicName);
                        console.log('🎯 Logo change:', originalSrc, '->', navbarLogoImg.src);
                        console.log('🏷️ Name change:', originalName, '->', navbarClinicNameEl.innerHTML);
                        
                        // Verify the change took effect
                        setTimeout(() => {
                            console.log('🔍 Verification check - current logo src:', navbarLogoImg.src);
                    ...
                    console.log('📋 Available clinic names:', Object.keys(clinicBranding));
                }
            }
                ...
                try {
                    ...
                    if (storedClinic) {
                        ...
                        if (ageInHours <= 24) {
                            ...
                            console.log('🕒 Stored clinic data is too old, clearing...');
            ...
            loadClinicFromSessionStorage() {
                try {
                    ...
                    if (storedClinic) {
                        ...
                        console.log('💾 Loaded clinic from session storage:', clinicData);
                        ...
                        console.log('✅ Restored clinic selection from session storage');
                    } else {
                        console.log('🌐 No stored clinic found, using general dashboard mode');
                        this.setGeneralDashboardMode();
                    }
                } catch (error) {
            ...
            updateDashboardForSelectedClinic(clinicData) {
                console.log('🏥 Updating dashboard for selected clinic:', clinicData);
            ...
            setGeneralDashboardMode() {
                console.log('🌐 Setting general dashboard mode');
            ...
            setGeneralNavbarBranding() {
                ...
                if (navbarLogoImg && navbarClinicNameEl) {
                    ...
                    console.log('🌐 Set general navbar branding');
                }
            }
            ...
            updateClinicSpecificUI(clinicData) {
                ...
                console.log('🎨 Updated clinic-specific UI for:', clinicData.name);
            }
            
            // Update clinic information footer
            updateClinicFooter(clinicData) {
                console.log('📄 Updating clinic footer with:', clinicData);
            ...
            // Set general footer information
            setGeneralFooter() {
                console.log('🌐 Setting general footer information');
        ...
        function showNotificationAlert(message, type) {
            ...
            console.log(`${alertStyle} ${message}`);
        ...
        // Debug: Check notification bell visibility
        window.addEventListener('load', function() {
            setTimeout(function() {
                const bell = document.getElementById('notification-bell');
                if (bell) {
                    console.log('Bell element found:', bell);
                    console.log('Bell visible:', window.getComputedStyle(bell).display !== 'none');
                    console.log('Bell position:', window.getComputedStyle(bell).position);
                    console.log('Bell color:', window.getComputedStyle(bell).color);
                    console.log('Bell font-family:', window.getComputedStyle(bell).fontFamily);
                } else {
