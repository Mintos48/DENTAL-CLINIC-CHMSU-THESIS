...
class PrescriptionController {
    ...
    public function createPrescription() {
        try {
            ...
            $stmt = $this->db->prepare("
                ...
                WHERE a.id = ? AND a.branch_id = ? AND a.status IN ('completed', 'in_progress')
            ");
            ...
            if (!$appointment) {
                return ['success' => false, 'message' => 'Appointment not found or not completed'];
            }
            ...
            if (!empty($medications)) {
                ...
                foreach ($medications as $index => $med) {
                    ...
                    $is_priority = $med['is_priority'] ?? false;
                    $sort_order = $index + 1;
    ...
    /**
     * Get appointments that need prescriptions (completed appointments without prescriptions)
     */
    ...
    public function updatePrescription() {
        try {
            ...
            ");
            $user_id = getSessionUserId();
            $stmt->bind_param('sisii', $diagnosis, $instructions, $follow_up_required, $follow_up_date, $user_id, $prescription_id);
            $stmt->execute();
